...
import * as CategoriesModel from "./model/resourceCategories";

const categoryDoc = v.object({
  _creationTime: v.number(),
  _id: v.id("resourceCategories"),
  active: v.boolean(),
...
export const listNonEmptyCategoryIds = query({
  ...
  handler: async (ctx, args) => {
    ...
    return results.filter((id): id is Id<"resourceCategories"> => id !== null);
  },
  returns: v.array(v.id("resourceCategories")),
});

export const listByParent = query({
  args: { parentId: v.optional(v.id("resourceCategories")) },
...
export const create = mutation({
  ...
  returns: v.id("resourceCategories"),
});
...
export const remove = mutation({
  args: { cascade: v.optional(v.boolean()), id: v.id("resourceCategories") },
  handler: async (ctx, args) => {
...
export const reorder = mutation({
  args: { items: v.array(v.object({ id: v.id("resourceCategories"), order: v.number() })) },
  handler: async (ctx, args) => {
