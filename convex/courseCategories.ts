...
import * as CategoriesModel from "./model/courseCategories";

const categoryDoc = v.object({
  _creationTime: v.number(),
  _id: v.id("courseCategories"),
  active: v.boolean(),
...
export const listNonEmptyCategoryIds = query({
  ...
  handler: async (ctx, args) => {
    ...
    return results.filter((id): id is Id<"courseCategories"> => id !== null);
  },
  returns: v.array(v.id("courseCategories")),
});

export const listByParent = query({
  args: { parentId: v.optional(v.id("courseCategories")) },
...
export const remove = mutation({
  args: { cascade: v.optional(v.boolean()), id: v.id("courseCategories") },
  handler: async (ctx, args) => {
...
export const reorder = mutation({
  args: { items: v.array(v.object({ id: v.id("courseCategories"), order: v.number() })) },
  handler: async (ctx, args) => {
