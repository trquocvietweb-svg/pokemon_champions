...
const LANDING_TYPE = 'use-case' as const;
const ROUTE_PATH = '/use-cases';
const BREADCRUMB_LABEL = 'Trường hợp sử dụng';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return buildSeoMetadata({
    contact,
    entity: { content: page.content, heroImage: page.heroImage, summary: page.summary, title: page.title },
    entityExists: true,
...
export default async function UseCaseLayout({ params, children }: Props) {
  const { slug } = await params;
  const client = getConvexClient();
  ...
  const schemas = [
    buildArticleSchema({ description: page.summary, image, publishedAt: page.publishedAt, siteName: site.site_name, title: page.title, updatedAt: page.updatedAt, url: pageUrl }),
